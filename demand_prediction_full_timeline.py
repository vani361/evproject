...
# Try to import XGBoost. If libomp is missing, fallback to RandomForest
...
def plot_importance(model, feature_names, target_col):
    ...
    # plt.show() # Commented out to prevent blocking in background execution
    plt.close() 
...
def run_demand_agent():
    ...
    for target_col, model_type in targets:
        ...
        df_model = prepare_urbanev_model_data(df, target_col=target_col).reset_index(drop=True)
        ...
        # Capture the raw predictions and scores from our updated function for the test set
        preds_test, scores = test_model(model, X_test, y_test, model_type, target_col)
        all_scores.append(scores)
        
        # Predict on the WHOLE timeline (100% of the data)
        X_all = df_model.drop(columns=[target_col], errors='ignore')
        if model_type == 'Regressor':
            preds_all = model.predict(X_all)
            preds_all = np.maximum(0.0, preds_all)
        elif model_type == 'Classifier':
            preds_all = model.predict_proba(X_all)[:, 1]
            
        # Attach the full timeline predictions to df_sorted
        df_sorted[f'predicted_{target_col}'] = preds_all
    ...
    # Save the whole timeline containing original columns and predicted columns
    preds_csv = os.path.join(SUBMISSION_DIR, "predicted_urban_ev_intervals_full.csv")
    df_sorted.to_csv(preds_csv, index=False)
